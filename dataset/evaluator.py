...
def PPLMetric(model, tokenizer, datasets, seq_len=128, batch_size = 4, device="cuda", mask_dict=None):
    metric = {}
    for dataset in datasets:
        _, test_loader = get_loaders(dataset, tokenizer, seq_len=seq_len, batch_size = batch_size)
        ppl = llama_eval(model, test_loader, device, mask_dict=mask_dict)
        metric[dataset] = ppl
        # print(metric)
    return metric
...
@torch.no_grad()
def llama_eval(model, test_lodaer, device, mask_dict=None):
    nlls = []
    n_samples = 0
    for batch in tqdm(test_lodaer):
        batch = batch.to(device)
        output = model(batch, mask_dict=mask_dict)
